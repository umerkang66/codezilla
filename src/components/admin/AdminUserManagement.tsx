...
export default function AdminUserManagement({
  ...
  return (
    <div className="space-y-6 text-left font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 bg-[#1A1A1A] border border-[#81D607]/30 rounded-2xl">
        ...
        <div className="relative min-w-[280px]">
          ...
          <input
            ...
            className="w-full bg-[#111111] border border-[#81D607]/40 text-[#E1E6EB] placeholder-[#9DA4B0] text-xs font-mono pl-9 pr-4 py-2.5 rounded-xl focus:outline-none focus:border-[#81D607]"
          />
        </div>
      </div>
      ...
      {!isSuperAdmin && (
        <div className="p-4 bg-[#111111] border border-[#81D607]/40 text-xs text-[#9DA4B0] flex items-start gap-3 rounded-xl">
      ...
      {message && (
        <div
          className={`p-3 text-xs font-mono flex items-center justify-between rounded-xl border ${
        ...
        >
          ...
          <button onClick={() => setMessage(null)} className="font-bold underline ml-4 cursor-pointer">
            Dismiss
          </button>
        </div>
      )}
      ...
      {/* Users Table */}
      <div className="border border-[#E1E6EB]/10 bg-[#1A1A1A] overflow-x-auto rounded-2xl overflow-hidden">
                ...
                return (
                  <tr key={u.id} className="hover:bg-[#111111]/50 transition-colors">
                    ...
                    <td className="py-3 px-4 font-mono font-bold">
                      <div className="flex items-center gap-3">
                        {u.avatar_url ? (
                          ...
                          <div className="w-7 h-7 bg-[#111111] border border-[#81D607]/40 flex items-center justify-center text-[#81D607] shrink-0 rounded-full">
                    ...
                    <td className="py-3 px-4">
                      {isUserMainAdmin ? (
                        <span className="px-2.5 py-1 bg-[#81D607] text-[#111111] font-mono font-extrabold text-[10px] uppercase rounded-full inline-flex items-center gap-1">
                          <Lock className="w-3 h-3" />
                          <span>Main Admin (Env)</span>
                        </span>
                      ) : isAdmin ? (
                        <span className="px-2.5 py-1 bg-[#111111] border border-[#81D607] text-[#81D607] font-mono font-bold text-[10px] uppercase rounded-full inline-flex items-center gap-1">
                          <ShieldCheck className="w-3 h-3 text-[#81D607]" />
                          <span>Admin</span>
                        </span>
                      ) : (
                        <span className="px-2 py-0.5 bg-[#111111] border border-[#E1E6EB]/15 text-[#9DA4B0] font-mono text-[10px] uppercase rounded-full">
                          User
                        </span>
                      )}
                    </td>
                    ...
                    <td className="py-3 px-4 text-right">
                      {isUserMainAdmin ? (
                        ...
                        <button
                          ...
                          disabled={isUpdating}
                          className={`px-3 py-1.5 font-mono font-bold text-[11px] transition-colors rounded-xl cursor-pointer ${
                            isAdmin
