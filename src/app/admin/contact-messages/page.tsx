...
export const metadata = {
  title: "Contact Messages | Codzilla Admin Panel",
  description: "View and manage incoming contact messages and project inquiries for Codzilla Technologies.",
};
...
export default async function AdminContactMessagesPage() {
        ...
        <div className="w-full max-w-md bg-[#1A1A1A] border border-red-500/60 p-8 space-y-6 text-center rounded-2xl shadow-2xl">
          ...
          <div className="flex flex-col gap-3 pt-2">
            <form action="/auth/signout" method="post">
              <button
                type="submit"
                className="w-full py-3 bg-red-600 hover:bg-red-700 text-[#E1E6EB] font-mono font-bold text-xs transition-colors rounded-xl cursor-pointer flex items-center justify-center gap-2"
              >
  ...
  // 4. Fetch initial contact messages list from Supabase
  ...
  if (messagesErr) {
    console.error("Error fetching initial contact messages:", messagesErr);
  }
